...
import com.livy.rpc.api.Job;
import com.livy.rpc.api.JobHandle;
import com.livy.rpc.api.LivyClient;
import com.livy.rpc.client.common.BufferUtils;
import com.livy.rpc.rsc.driver.AddFileJob;
import com.livy.rpc.rsc.driver.AddJarJob;
import com.livy.rpc.rsc.rpc.Rpc;

import static com.livy.rpc.rsc.RSCConf.Entry.*;
...
public class RSCClient implements LivyClient {
  ...
  RSCClient(RSCConf conf, Promise<ContextInfo> ctx, Process driverProcess) throws IOException {
    ...
    this.jobs = new ConcurrentHashMap<String, JobHandleImpl<?>>();
  ...
  private class ClientProtocol extends BaseProtocol {
    ...
    <T> JobHandleImpl<T> submit(Job<T> job) {
      ...
      final Promise<T> promise = eventLoopGroup.next().newPromise();
      final JobHandleImpl<T> handle = new JobHandleImpl<T>(RSCClient.this, promise, jobId);
